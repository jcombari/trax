...
def Favor(d_feature, n_heads=1, dropout=0.0,
  ...
  """Returns a layer that maps activations to activations, no masking.
  ...
  def bidirectional_numerator(query_prime, key_prime, value):
    kvs = jnp.einsum('lbhm,lbhd->bhmd', key_prime, value)
    return jnp.einsum('lbhm,bhmd->lbhd', query_prime, kvs)

  def bidirectional_denominator(query_prime, key_prime):
    all_ones = jnp.ones([query_prime.shape[0]])
    ks_sum = jnp.einsum('lbhm,l->bhm', key_prime, all_ones)
    return jnp.einsum('lbhm,bhm->lbh', query_prime, ks_sum)
